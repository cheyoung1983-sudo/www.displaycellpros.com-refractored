...
export async function addToCart(variantId: string): Promise<Cart> {
  ...
  const data = await shopifyFetch<{ cartLinesAdd: { cart: Cart } }>(ADD_TO_CART_MUTATION, {
    cartId,
...
export async function updateCartLine(lineId: string, quantity: number): Promise<Cart> {
  ...
  const data = await shopifyFetch<{ cartLinesUpdate: { cart: Cart } }>(UPDATE_CART_MUTATION, {
    cartId,
...
export async function removeFromCart(lineId: string): Promise<Cart> {
  ...
  const data = await shopifyFetch<{ cartLinesRemove: { cart: Cart } }>(REMOVE_FROM_CART_MUTATION, {
    cartId,
    lineIds: [lineId],
  });
