...
export default function App() {
  ...
  const fetchSdStatus = async (retries = 2, delay = 1000) => {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt === retries) {
          console.warn("Failed to fetch Service Directory status, using simulated fallback:", err);
          setSdStatus({
            active: false,
            mode: "simulated",
            usingFallback: true,
            error: null,
            message: "Using Local Service Directory Registry simulation layer (Safe Sandbox). No GCP Service Account permissions required."
          });
        } else {
          await new Promise((resolve) => setTimeout(resolve, delay));
        }
  ...
  const handleListNamespaces = async (proj = sdProjectId, loc = sdLocationId, retries = 2, delay = 1000) => {
    setSdLoading(true);
    setSdError(null);
    setSdSuccess(null);
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt === retries) {
          console.warn("Network error fetching namespaces, using simulated fallbacks:", err);
          const fallbackNamespaces = [
            { name: `projects/${proj}/locations/${loc}/namespaces/spokane-lab-networks` },
            { name: `projects/${proj}/locations/${loc}/namespaces/seattle-fleet-systems` },
            { name: `projects/${proj}/locations/${loc}/namespaces/billing-relays` }
          ];
          setSdNamespaces(fallbackNamespaces);
          if (fallbackNamespaces.length > 0) {
            const firstNs = fallbackNamespaces[0].name;
            setSelectedNamespace(firstNs);
            handleListServices(firstNs);
          }
        } else {
...
Status: ${issueType === "battery" ? "DEGRADED" : "OPTIMAL"}`;
                ...
                <div>
                  ...
                  <nav className="space-y-1">
                    ...
                    <button
                      onClick={() => setLabTab("verification")}
                      className={`w-full flex items-center justify-between p-2.5 rounded-lg text-xs font-semibold transition-all ${
                        labTab === "verification" 
                          ? "bg-blue-600 text-white shadow-md" 
                          : "text-slate-300 hover:bg-slate-800"
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span>OAuth Verification Lab</span>
                      </div>
                      <span className="px-1.5 py-0.2 text-[9px] rounded font-mono bg-amber-900/40 text-amber-300 font-bold">
                        ACTION
                      </span>
                    </button>
                  ...
                  <section className="bg-slate-800 border border-slate-700 rounded-xl flex flex-col flex-1 shadow-md overflow-hidden animate-in fade-in duration-200 p-5 space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h2 className="text-sm font-bold text-slate-100 tracking-wider uppercase font-mono">
                            Google OAuth Domain Verification Lab
                          </h2>
                          <p className="text-[10px] text-slate-400 font-mono">Resolve homepage domain ownership verification issues for Google Trust & Safety review</p>
                        </div>
                      </div>
                      <span className="text-[9px] bg-amber-900/40 text-amber-300 font-bold px-2 py-0.5 rounded border border-amber-800/30 font-mono uppercase tracking-widest animate-pulse">
                        Action Required
                      </span>
                    </div>

                    {/* Trust and Safety Notification Card */}
                    <div className="bg-amber-950/20 border border-amber-500/20 rounded-lg p-4 space-y-3">
                      <div className="flex items-center gap-2 text-amber-400 font-bold text-xs uppercase font-mono">
                        ...
                        <span>Homepage Ownership Verification Issue</span>
                      </div>
                      <div className="text-xs text-slate-300 leading-relaxed space-y-2 font-mono">
                        <p>
                          Google's Trust and Safety team has paused your OAuth Consent Screen verification because your homepage website (<code className="text-amber-300 font-bold bg-slate-950 px-1 py-0.2 rounded">https://displaycellpros.com</code>) is not verified as registered to you.
                        </p>
                        <p className="text-[11px] text-slate-400">
                          To resume verification, you must prove ownership of this domain in Google Search Console using either <b>HTML File Upload</b> or <b>HTML Meta Tag</b>.
                        </p>
                      </div>
                    </div>
