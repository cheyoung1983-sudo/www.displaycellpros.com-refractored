...
async function startServer() {
  ...
  // ── Aurora IAM Connectivity Health Probe ──────────────────────────────────
  // GET /api/test-db
  // Verifies the RDS Signer / OIDC token flow and pool connectivity end-to-end.
  // Returns the Aurora server timestamp on success; detailed error on failure.
  app.get("/api/test-db", async (req, res) => {
    try {
      const { getDbPool } = await import("./db.ts");
      const pool = getDbPool();
      const result = await pool.query("SELECT NOW() AS aurora_time");
      res.json({
        status: "FORENSIC_POOL_NOMINAL",
        auth_mode: process.env.AWS_ROLE_ARN ? "IAM_OIDC" : "PASSWORD",
        aurora_time: result.rows[0]?.aurora_time,
